...
import { Container, type SelectItem, Text } from "@earendil-works/pi-tui";
	...
	pi.registerCommand("m", {
		...
		handler: async (args, ctx) => {
			...
			const items: SelectItem[] = sorted.map((m) => {
				const isActive =
					currentModel && m.provider === currentModel.provider && m.id === currentModel.id;
				return {
			...
			const chosen = await ctx.ui.custom<string | null>((tui, thm, _kb, done) => {
				...
				container.addChild(
					new Text(
						thm.fg("accent", thm.bold(`Select Model  (${available.length} available)`)),
						1,
						0,
					),
				);
				...
				container.addChild(
					new Text(
						thm.fg("dim", "↑↓ navigate  •  type to filter  •  backspace to clear  •  enter select  •  esc cancel"),
						1,
						0,
					),
				);
