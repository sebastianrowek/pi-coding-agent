...
- `--bash-guard-disable`: completely disables bash-guard (no prompting, no blocking) in both main
  and subagent modes. Also settable via the `BASH_GUARD_DISABLE=1` env var, which is the convenient
  way to disable it in spawned subagent sessions where flags are harder to pass.
