...
// Disable switch. Honours both the --bash-guard-disable flag (main session) and the
// BASH_GUARD_DISABLE env var (works in subagent sessions, where flags are less convenient).
function isDisabled(pi: ExtensionAPI): boolean {
	const env = process.env.BASH_GUARD_DISABLE;
	if (env && env !== "0" && env.toLowerCase() !== "false") return true;
	return pi.getFlag("--bash-guard-disable") === true;
}

export default function (pi: ExtensionAPI) {
	pi.registerFlag("bash-guard-disable", {
		description: "Completely disable bash-guard (no prompting or blocking). Also settable via BASH_GUARD_DISABLE=1.",
		type: "boolean",
		default: false,
	});
