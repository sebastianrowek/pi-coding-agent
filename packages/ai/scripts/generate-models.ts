...
async function fetchOpenRouterModels(): Promise<Model<any>[]> {
	try {
		...
		// Return fallback OpenRouter models when API fetch fails
		console.log("Using fallback OpenRouter models...");
		return [
			// OpenRouter backend models - these are upstream models that OpenRouter exposes
			// Anthropic backend
			{
			...
			// Additional models that may be used in tests
			{
			...
			},
			{
				...
				reasoning: true,
				input: ["text"],
				cost: { input: 0.09, output: 0.18, cacheRead: 0.02, cacheWrite: 0 },
				contextWindow: 1048576,
				maxTokens: 65536,
			},
			{
